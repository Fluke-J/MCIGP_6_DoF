...
class GraspRectangles:
    ...
    def __getattr__(self, attr):
        ...
        if hasattr(GraspRectangle, attr):
            member = getattr(GraspRectangle, attr)
            if callable(member):
...
class GraspRectangle:
    ...
    def plot(self, ax, color=None):
        ...
        # linewidth_og = 1.8
        ax.plot(points[:, 1], points[:, 0], color=color, linewidth=5)
...
class Grasp:
    ...
    def __init__(self, center, angle, length=60, width=1):
        self.center = center
...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1, point=None, fine_point=None, fine_width=None):
    """
    ...
    if point is not None:
        grasps = []
        best_point_array = point
        best_point = tuple(best_point_array)
        best_angle = ang_img[best_point]
        g = Grasp(best_point, best_angle)

        if width_img is not None:
            g.length = width_img[best_point]
            g.width = g.length / 5
        if fine_point is None:
            grasps.append(g)

        if fine_point is not None:
            f_point_array = fine_point
            fine_point = tuple(f_point_array)
            fine_angle = best_angle
            g = Grasp(fine_point, fine_angle)
            g.length = fine_width
            g.width = g.length / 5
            grasps.append(g)

        local_max = peak_local_max(q_img, min_distance=1, threshold_abs=0.1, num_peaks=no_grasps)
